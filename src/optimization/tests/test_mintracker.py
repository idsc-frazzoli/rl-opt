...
class MinTrackerTest(unittest.TestCase):
    ...
    def test_discardable(self):
        ...
        self.assertTrue(mintracker.discardable(x1, x2, 1))
        self.assertFalse(mintracker.discardable(x1, x3, 1))
        self.assertTrue(mintracker.discardable(x3, x2, 1))

    def test_update_exact(self):
        slack = [0.1, 0.2]
        mintracker = ExactMinTracker(slack)
        x1 = np.array([1, 1])
        mintracker.update_mintracker(x1)
        print(mintracker.candidates)
        x2 = np.array([0.98, 1.3])
        mintracker.update_mintracker(x2)
        print(mintracker.candidates)
    ...
    def test_within_hypercuboid(self):
        slack = [0.1, 0.2, 0.1]
        ...
        self.assertTrue(mintracker.within_hypercuboid(x1, x2))
        self.assertFalse(mintracker.within_hypercuboid(x1, x3))
